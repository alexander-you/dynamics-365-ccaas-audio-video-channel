...
via the `sdkUrl` input (default: the same-origin web-resource path `/WebResources/alex_acv_acs_sdk.js`).
...
- **Same-origin web resource (preferred for CSP, needs an org setting):** raise the org
  ...
  then upload `acv-acs-sdk.js` as a JavaScript web resource named e.g. `alex_acv_acs_sdk.js`. This keeps the
  default `sdkUrl` and avoids any cross-origin CSP change. *(Org-setting change — reviewer/admin decision.)*
  For the Demo Contact Center EN POC this was set to **10 MB (10,240 KB)**; rollback is **5,120 KB**.
